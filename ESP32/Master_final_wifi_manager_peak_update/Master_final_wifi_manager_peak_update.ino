...
void setup(){
  ...
  myOLED.setFont(SmallFont);
  myOLED.invert(false);
  myOLED.print(".CONNECTING.",34,24);
  //myOLED.print("LOW BALANCE",34,24);
  myOLED.print("SCANNING  WI-FI",23,39);
  //myOLED.print("PLEASE RECHARGE",23,39);
  myOLED.update();
  //delay(1000);
  Serial.println("START");
  bool res;
  res = wm.autoConnect("Smart_Energy_Meter","18143122027");
  if(!res) {
...
//NTP server added
  ...
  if(theft == 1){
    ...
    myOLED.clrScr();
    myOLED.setFont(SmallFont);
    myOLED.print("THEFT DETECT",34,24);
    myOLED.print("SYSTEM TURN-OFF",23,39);
    myOLED.update();
    delay(1000*120);
    ESP.restart();
  }
  if(theft == 0 && rec_kwh > 0){
    nor_display();
  }
